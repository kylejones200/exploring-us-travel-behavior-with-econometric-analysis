...
```python
"""
Gas Price vs. Travel Behavior Analysis
Econometric analysis of U.S. transportation data (2018-2025)
Data source: U.S. Bureau of Transportation Statistics
Analysis: OLS regression with controls, structural break tests
"""
import pandas as pd
import numpy as np
...
def load_data(filepath):
    """Load and preprocess BTS transportation data."""
    ...
    df.columns = ['Date', 'Miles_Traveled', 'Gas_Price', 'Unemployment', 'GDP']
    ...
    df['Unemployment'] = clean_numeric(df['Unemployment'])
...
def create_features(df):
    """Create log-transformed and time-based features."""
    df['log_miles'] = np.log(df['Miles_Traveled'])
    df['log_gas'] = np.log(df['Gas_Price'])
    ...
    df['COVID'] = (df['Date'] >= covid_date).astype(int)
    
    def month_diff(d1, d2):
        return (d1.year - d2.year) * 12 + d1.month - d2.month
    
    df['t'] = df['Date'].apply(lambda d: month_diff(d, covid_date))
    df['spline1'] = df['t']
    df['spline2'] = (df['t'] > 0) * df['t']
    df['spline1_sq'] = df['spline1'] ** 2
    df['spline2_sq'] = df['spline2'] ** 2
    
    return df, month_dummies

def run_naive_model(df):
    """Model 1: Naive regression without controls."""
    X = sm.add_constant(df[['Gas_Price']])
    y = df['Miles_Traveled']
    return sm.OLS(y, X).fit()

def run_controlled_model(df, month_dummies):
    """Model 2: Log-log with unemployment and month fixed effects."""
    X = pd.concat([df[['log_gas', 'Unemployment']], month_dummies], axis=1)
    X = sm.add_constant(X)
    y = df['log_miles']
    return sm.OLS(y, X.astype(float)).fit()

def run_gdp_model(df, month_dummies):
    """Model 3: Add GDP to separate employment from general economic activity."""
    X = pd.concat([df[['log_gas', 'Unemployment', 'log_gdp']], month_dummies], axis=1)
    X = sm.add_constant(X)
    y = df['log_miles']
    return sm.OLS(y, X.astype(float)).fit()

def run_covid_model(df, month_dummies):
    """Model 4: Test for COVID structural break."""
    X = pd.concat([
        df[['log_gas', 'Unemployment', 'COVID']],
        (df['log_gas'] * df['COVID']).rename('log_gas_COVID'),
        month_dummies
    ], axis=1)
    X = sm.add_constant(X)
    y = df['log_miles']
...
def run_spline_model(df):
    """Model 5: Quadratic spline regression for COVID break."""
    X = df[['spline1', 'spline1_sq', 'spline2', 'spline2_sq', 'Unemployment']]
    X = sm.add_constant(X)
    y = df['log_miles']
    model = sm.OLS(y, X).fit()
    df['fitted_spline'] = model.fittedvalues
    return model
...
def create_visualizations(df, output_dir):
    ...
    # Figure 1: Gas price vs miles scatter
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.scatter(df['Gas_Price'], df['Miles_Traveled'] / 1e9, 
               color='black', alpha=0.6, s=30)
    ax.set_xlabel('Gas Price ($/gallon)')
    ax.set_ylabel('Vehicle Miles Traveled (billions)')
    ax.set_title('Gas Price vs. Miles Traveled')
    plt.savefig(output_dir / 'fig1_gas_vs_miles.png')
    plt.close()
    
    # Figure 2: Time series with COVID break
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(df['Date'], df['log_miles'], label='Observed', color='black', lw=1.5, alpha=0.7)
    ax.plot(df['Date'], df['fitted_spline'], label='Spline Fit', color='gray', linestyle='--', lw=2)
    ax.axvline(pd.to_datetime('2020-03-01'), color='red', linestyle=':', lw=2, alpha=0.7, label='COVID-19')
    ax.set_xlabel('Date')
    ax.set_ylabel('Log(Vehicle Miles Traveled)')
    ax.set_title('Time Series with COVID Structural Break')
    ax.legend(frameon=False)
    plt.savefig(output_dir / 'fig2_time_series_covid.png')
    plt.close()
    
    # Figure 3: Unemployment vs miles
    fig, ax = plt.subplots(figsize=(8, 5))
    pre_covid = df['Date'] < pd.to_datetime('2020-03-01')
    ax.scatter(df.loc[pre_covid, 'Unemployment'], df.loc[pre_covid, 'log_miles'],
               color='black', alpha=0.5, s=30, label='Pre-COVID')
    ax.scatter(df.loc[~pre_covid, 'Unemployment'], df.loc[~pre_covid, 'log_miles'],
               color='red', alpha=0.6, s=30, label='COVID Era')
    
    z = np.polyfit(df['Unemployment'], df['log_miles'], 1)
    p = np.poly1d(z)
    x_line = np.linspace(df['Unemployment'].min(), df['Unemployment'].max(), 100)
    ax.plot(x_line, p(x_line), color='gray', linestyle='--', lw=2, label='Linear Fit')
    
    ax.set_xlabel('Unemployment Rate (%)')
    ax.set_ylabel('Log(Vehicle Miles Traveled)')
    ax.set_title('Unemployment Rate vs. Miles Traveled')
    ax.legend(frameon=False)
    plt.savefig(output_dir / 'fig3_unemployment_vs_miles.png')
    plt.close()

def export_results(df, models, output_dir):
    """Export summary statistics and model results."""
    output_dir.mkdir(exist_ok=True)
    
    summary_stats = df[['Miles_Traveled', 'Gas_Price', 'Unemployment', 'GDP']].describe()
    summary_stats.to_csv(output_dir / 'summary_statistics.csv')
    
    corr_matrix = df[['Miles_Traveled', 'Gas_Price', 'Unemployment', 'GDP']].corr()
    corr_matrix.to_csv(output_dir / 'correlation_matrix.csv')
    
    with open(output_dir / 'model_results.txt', 'w') as f:
        for i, (name, model) in enumerate(models.items(), 1):
            f.write(f"MODEL {i}: {name}\n")
            f.write("-" * 80 + "\n")
...
def main():
    ...
    # Configuration
    data_file = Path('data/Monthly_Transportation_Statistics_20251110.csv')
    output_dir = Path('outputs')
    
    # Load and prepare data
    df = load_data(data_file)
    df, month_dummies = create_features(df)
    
    # Run models
    model_naive = run_naive_model(df)
    model_controlled = run_controlled_model(df, month_dummies)
    model_gdp = run_gdp_model(df, month_dummies)
    model_covid = run_covid_model(df, month_dummies)
    model_spline = run_spline_model(df)
    
    models = {
        'Naive Regression': model_naive,
        'Controlled Model': model_controlled,
        'Model with GDP': model_gdp,
        'COVID Model': model_covid,
        'Spline Model': model_spline
    }
    
    # Generate outputs
    create_visualizations(df, output_dir)
    export_results(df, models, output_dir)
    
    # Print summary
    print(f"Analysis complete. {len(df)} observations processed.")
    print(f"Key finding: Unemployment coefficient = {model_gdp.params['Unemployment']:.4f}")
    print(f"             (p-value: {model_gdp.pvalues['Unemployment']:.2e})")
    print(f"             Gas price elasticity = {model_gdp.params['log_gas']:.4f}")
    print(f"             (p-value: {model_gdp.pvalues['log_gas']:.3f})")
